...
while (time.time() < timeout):
	...
	if(len(frames_arrived) > current_frame):
		...
		print "Quantity of non outlier (F1, F2, F3)"
		print str(f1_nonoutlier) + " " + str(f2_nonoutlier) + " " + str(f3_nonoutlier) 
		print "Quantity of outlier (F1, F2, F3)"
		print str(f1_outlier ) + " " + str(f2_outlier) + " " + str(f3_outlier)
